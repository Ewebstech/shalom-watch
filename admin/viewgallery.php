...
<html lang="en">
  <head>
	...
	{
		$show = mysql_query("update items set status='show' where itemtype='gallery' and id='$_REQUEST[show]'");
		if($show) { 		
		header("location: viewgallery.php"); 
		}
	}
	...
	{
		$hide = mysql_query("update items set status='hide' where itemtype='gallery' and id='$_REQUEST[hide]'");
		if($hide) { 		
		header("location: viewgallery.php"); 
		}
	}
	...
	?>
	  ...
	  <?php $viewcopier = mysql_query("select * from items where itemtype='gallery' order by time desc"); ?>
	  <?php $viewcopiernum = mysql_query("select * from items where itemtype='gallery' order by time desc"); ?>
          <section class="wrapper">
		  <div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-files-o"></i> View Gallery Uploads</h3>
					<ol class="breadcrumb">
						<li><i class="fa fa-home"></i><a href="index.php">Home</a></li>
						<li><i class="icon_document_alt"></i>View Gallery Uploads</li>
						<?php if(!isset($_REQUEST["viewproduct"])) { ?>
						<li><span class="label label-success"><b><?php echo $r = mysql_num_rows($viewcopiernum); ?></b> Gallery Items are Currently Active</span>
						<?php } ?>
					</ol>
				</div>
			</div>
				 ...
				 <table id="example1" class="table table-striped table-advance table-bordered table-hover" style='text-align: center;'>
							<thead >
							 ...
							 <?php while($view = mysql_fetch_assoc($viewcopier)) { ?>
                              <tr>
								 <td><p><?php if($view["status"] == "show") { echo"<span class='label label-primary'>Active</span>"; } ?></p>
                                ...
                                <td>
                                  <div class="btn-group" >
									   ...
									   <a class="btn btn-primary" href="viewgallery.php?show=<?php echo"$view[id]"; ?>" data-toggle='tooltip' title='Show Ad'> Display <i class="icon_plus_alt2"></i></a><br /><br>
										<?php } else { ?>
										 <a class="btn btn-primary" href="viewgallery.php?hide=<?php echo"$view[id]"; ?>" data-toggle='tooltip' title='Hide Ad'> Hide <i class="icon_plus_alt2"></i></a><br /><br>
										<?php } ?>
                                      <a class="btn btn-primary" href="viewgallery.php?viewproduct=<?php echo"$view[id]"; ?>" data-toggle='tooltip' title='View Product'> View <i class="icon_plus_alt2"></i></a><br /><br>
                                      
                                      <a class="btn btn-danger" href="viewgallery.php?deleteitem=<?php echo"$view[id]"; ?>" data-toggle='tooltip' title='Delete Item'>Delete <i class="icon_close_alt2"></i></a>
                                  </div>
                                  </td>
                              </tr>
							 <?php } ?>
